  ...
  const fetchAnnouncements = async () => {
    try {
      const response = await axios.get(
        "http://localhost:1337/api/anakoinoseis"
      );
              ...
              {subset.map((card) => (
                ...
                >
                  <div className="p-6 bg-white rounded-t-lg">
                    <h5 className="mb-2 lg:text-2xl font-bold leading-tight ">
                          ...
                          components={{
                            a: ({ node, children, ...props }) => (
                              <div className="flex justify-center">
                                <a
                                  className="no-underline  text-blue-600 hover:text-blue-700 underline mt-5 "
                                  href={props.href}
                                  target="_blank"
                                >
                                  <span>
                    ...
                    <div>
                      {card &&
                        card.attributes &&
                        card.attributes.Title &&
                        card.attributes.announcement &&
                        card.attributes.has_announcement && (
                          <Link
                            ...
                            state={{
                              announcementData: {
                                ...
                                announcement: card.attributes.announcement,
                              },
                            }}
                          >
                  ...
                  <div className="border-t-2 border-neutral-100 px-6 py-3 bg-white rounded-b-lg">
                    <span className="flex justify-center items-center text-black">
                      <FcCalendar className="mr-1" />
                      <span>
                        {card.attributes.date
                          .replace("Δημιουργήθηκε :", "")
                          .trim()}
                      </span>
                    </span>
                  </div>
